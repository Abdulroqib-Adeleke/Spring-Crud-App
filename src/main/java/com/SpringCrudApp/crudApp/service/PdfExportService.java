...
public class PdfExportService {
    ...
    public void export(HttpServletResponse response) {
        ...
        Document document = new Document(
                PageSize.A4.rotate(), 30, 30, 40, 40);
        try {
            PdfWriter writer = PdfWriter.getInstance(document, response.getOutputStream());
            writer.setPageEvent((PdfPageEvent) new PageFooterEvent());
            document.open();

            com.itextpdf.text.Font companyFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 16);
            com.itextpdf.text.Font titleFont   = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 13);
            com.itextpdf.text.Font metaFont    = FontFactory.getFont(FontFactory.HELVETICA, 10);
            ...
            com.itextpdf.text.Font inactiveFont   = FontFactory.getFont(FontFactory.HELVETICA, 8);

            int rowNum = 0;
            for (Employee emp : employees) {
                boolean isActive = Boolean.TRUE.equals(emp.getActive());
                BaseColor bg   = (rowNum % 2 == 0) ? ROW_WHITE : ROW_ALT;
                com.itextpdf.text.Font font = isActive ? dataFont : inactiveFont;
